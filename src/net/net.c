...
static void rtl_poll(void) {
    ...
    if (isr) outw(rtl_base + RTL_ISR, isr);
...
static void pn_poll(void) {
    ...
    for (int n = 0; n < PN_RX; n++) {
        ...
        if (d->status & PD_OWN) break;
        uint32_t st = d->status;
        uint32_t mcnt = d->misc & 0x0FFF;
    ...
    if (uptime_ticks - last_link_tick >= 500) {
...
static void e1000_init_chip(void) {
    ...
    e1000_w(E1000_RAH0, (rah & 0xFFFF) | (1u << 31));
...
static void e1000_send(const uint8_t* data, uint32_t len) {
    ...
    for (int i = 0; i < 20 && !(d->status & 1); i++) delay_ms(1);
...
static void e1000_poll(void) {
    ...
    for (int n = 0; n < E1000_NRX; n++) {
        ...
        if ((d->status & 2) && !d->errors && len >= 14 && len <= 1514) {
            if (net_rx_cb) net_rx_cb(e1000_rxb[e1000_rx_head], len);
        }
        d->status = 0;
        e1000_w(E1000_RDT, (uint32_t)e1000_rx_head);
    ...
    for (uint32_t bus = 0; bus < 256; bus++) {
        for (uint32_t slot = 0; slot < 32; slot++) {
            for (uint32_t func = 0; func < 8; func++) {
                ...
                if (func == 0 && (hdr & 0x00800000) == 0) break;
            }
        }
    }
...
static void icmp_recv(const uint8_t* payload, uint32_t len, const uint8_t* src_ip) {
    ...
    if (payload[4] != 0x12 || payload[5] != 0x34) return;
...
static uint8_t tcp_rx_buf[65536];
static volatile uint32_t tcp_rx_len = 0;
static uint32_t tcp_rx_rd = 0;
...
int net_tcp_open(const uint8_t* ip, uint16_t port) {
    ...
    for (int w = 0; w < 300; w++) {
        ...
        if (tcp_reset) break;
        if (w == 100 || w == 200) tcp_send_flags(0x02, NULL, 0);
    }
    if (!connected) {
        tcp_cb = NULL;
...
static uint32_t dhcp_bound_tick = 0;
static uint32_t dhcp_next_tick = 0;
static uint32_t dhcp_backoff_ms = 1000;
static int      dhcp_tries = 0;
static int      dhcp_had_link = 0;
static int      dhcp_verbose = 0;
...
static void dhcp_send(int type, const uint8_t* req_ip, const uint8_t* srv, int unicast) {
    ...
    d[o++] = 55; d[o++] = 5; d[o++] = 1; d[o++] = 3; d[o++] = 6; d[o++] = 51; d[o++] = 54;
    d[o++] = 57; d[o++] = 2; d[o++] = 0x02; d[o++] = 0x40;
    d[o++] = 0xFF;
    ...
    uint16_t tlen = (uint16_t)(20 + udplen);
    pkt[off + 2] = (tlen >> 8) & 0xFF; pkt[off + 3] = tlen & 0xFF;
    pkt[off + 4] = (uint8_t)(dhcp_xid >> 8); pkt[off + 5] = (uint8_t)dhcp_xid;
...
static void dhcp_apply_lease(void) {
    ...
    if (!net_dns[0] && !net_dns[1] && !net_dns[2] && !net_dns[3]) {
        for (int i = 0; i < 4; i++) net_dns[i] = net_gw[i];
    }
    net_configured = 1;
    dhcp_bound_tick = uptime_ticks;
    if (dhcp_lease_secs < 60) dhcp_lease_secs = 60;
    if (dhcp_lease_secs > 30u * 86400u) dhcp_lease_secs = 30u * 86400u;
    dhcp_state = DHCP_BOUND;
    dhcp_next_tick = uptime_ticks + (dhcp_lease_secs / 2) * TICKS_PER_SEC;
...
static void dhcp_recv(const uint8_t* payload, uint32_t len, const uint8_t* src_ip) {
    ...
    if (my_memcmp(&payload[28], net_mac, 6) != 0) return;
    ...
    if (msgtype == 2 && dhcp_state == DHCP_SELECTING) {
        for (int i = 0; i < 4; i++) {
...
static void dhcp_use_linklocal(void) {
    ...
    dhcp_next_tick = uptime_ticks + 60 * TICKS_PER_SEC;
}
...
static void dhcp_tick(void) {
    ...
    if (net_has_link != dhcp_had_link) {
        ...
        if (net_has_link) {
            dhcp_log("link up");
            dhcp_state = DHCP_INIT;
            dhcp_next_tick = uptime_ticks + 200;
        } else {
            dhcp_log("link down");
            net_configured = 0;
            dhcp_state = DHCP_INIT;
