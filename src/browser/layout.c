...
static void downsample(uint32_t* px, int w, int h, int nw, int nh) {
    for (int dy = 0; dy < nh; dy++) {
        ...
        for (int dx = 0; dx < nw; dx++) {
            ...
            if (!n) n = 1;
            px[(size_t)dy * nw + dx] = ((a / n) << 24) | ((r / n) << 16) | ((g / n) << 8) | (b / n);
        }
    }
}
...
static void text_metrics(const br_style_t* st, tm_t* m) {
    ...
    if (m->lh < m->ascent + m->descent - 2) m->lh = m->ascent + m->descent - 2;
    if (m->lh < 4) m->lh = 4;
}
...
static void flow_newline(flow_t* f) {
    ...
    f->any = 0;
    if (f->x1_full) { f->x1 = f->x1_full; f->x1_full = 0; }
}
...
static int is_inline_node(const br_node_t* c) {
    ...
    if (c->style.float_dir == 3) return 0;
    if (c->style.float_dir && d != BR_DISPLAY_TABLE) return 1;
...
static void flow_inline_block(br_node_t* c, flow_t* f) {
    ...
    if (fixed) {
        ...
        bw = natural_width(c);
        box_count = save;
        if (pct_cap > 0 && bw > pct_cap) bw = pct_cap;
    ...
    if (saved_mw < -1) c->style.max_width = -1;
...
static void flow_image(br_node_t* c, flow_t* f) {
    ...
    else if (c->style.width < -1) { w = (f->x1 - f->x0) * (-(c->style.width) - 2) / 100; if (h && !wa) h = 0; }
...
static int layout_table(br_node_t* t, int x, int y, int width) {
    ...
    if (ib_depth == 0 && t->parent && !measuring) {  }
    ...
    t->used_w = natural;
    if (t->style.width == -1 && natural < tw) tw = natural;
    {
        ...
        if (nauto == 0 || for_auto < 20 * nauto) {
            ...
            for (int i = 0; i < cols; i++) {
                ...
                if (spare <= 0) colw[i] = auto_nat > 0 ? for_auto * colnat[i] / auto_nat : for_auto / nauto;
...
static int layout_block(br_node_t* n, int x, int y, int width, int list_index) {
    ...
    if (br_stack_headroom() < BR_STACK_MIN) return 0;
    ...
    if (br_streq(n->tag, "table") || st->display == BR_DISPLAY_TABLE) {
        int th = layout_table(n, cx, cy, cw);
