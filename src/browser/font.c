...
static void outline_glyph(const face_t* f, int glyph, const xform_t* m) {
    ...
    if (next <= off || next > f->glyf_len || next - off < 10) return;
    ...
    for (int iter = 0; iter < 16; iter++) {
        ...
        if (flags & 8) { if (p + 2 > gend) return; sub.a = sub.d = rds16(p) << 2; p += 2; }
...
static int32_t acc[(RAST_MAX_W + 2) * RAST_MAX_H];
...
static void rast_edge(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int w, int h) {
    ...
    for (int row = row0; row <= row1; row++) {
        ...
        if (xi0 >= w) { line[w] += (int32_t)cov; continue; }
        if (xi0 == xi1) {

            int64_t xm = ((xa + xb) >> 1) - ((int64_t)xi0 << 16);
            ...
            int64_t span = xb - xa;
            int64_t slope = (cov << 16) / span;

            int64_t fa = (((int64_t)(xi0 + 1) << 16) - xa);
            int64_t c0 = (slope * fa) >> 16;
...
static int glyph_for(int face, uint32_t cp) {
    ...
    if (!g && cp > 0x7F) {
        ...
        else if (cp == 0x200B || cp == 0xFEFF || cp == 0x00AD) return -1;
        else if (face >= 6) {
...
static void blit_glyph(const glyph_cache_t* e, int px, int py, uint32_t fg, const w98_rect_t* clip, int synth_bold, int synth_italic) {
    ...
    for (int y = 0; y < e->h; y++) {
        ...
        int shear = synth_italic ? (py - sy) / 4 : 0;
        for (int x = 0; x < ew; x++) {
            int sx = x0 + x + shear;
